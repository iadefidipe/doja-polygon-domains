...
contract Domains is ERC721URIStorage {
    ...
    mapping(string => address) public domains; // maps a string of name to 
    mapping(string => Record) public records; // records attatched to a domain
...
error InvalidName(string name);
  ...
  constructor( string memory _tld ) payable  ERC721("Doja Name Service", "DNS")  {
  ...
  //get the price of the domain based on the lenght 
  ...
  function setRecord( string calldata name, string calldata _email,string calldata _website,string calldata _description,string calldata _notice) public{
      if( domains[name] != msg.sender ) revert Unauthorized();

      Record memory domainRecord = Record({
          email: _email,
          website: _website,
